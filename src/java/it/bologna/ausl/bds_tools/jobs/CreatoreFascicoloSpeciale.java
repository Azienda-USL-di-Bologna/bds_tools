...
public class CreatoreFascicoloSpeciale implements Job {
    ...
    private final String fascicoloSpecialeRaccoltaSemplice;
    ...
    public CreatoreFascicoloSpeciale() {
        log.info("=========================== Inizializzazione Servizio Creazione Fascicoli Speciali ===========================");
        ...
        fascicoloSpecialeRaccoltaSemplice = nomeFascicoliSplittati[i++];
        ...
        log.info("=========================== Fine Inizializzazione Servizio Creazione Fascicoli Speciali ===========================");
    }
    ...
    private void create() throws NullPointerException, SQLException, ServletException {
        ...
        createFascicoloSpeciale("SFSpecialeRS" + anno, 4, "2", fascicoloSpecialeRaccoltaSemplice, idFascicoloSpecialeAtti);
...
//            ps.setDate(i++, new Date(formatter.parseDateTime("01/01/2022 00:00:00").toInstant().getMillis()));
//            ps.setDate(i++, new Date(formatter.parseDateTime("01/01/2022 00:00:00").toInstant().getMillis()));
...
//            log.error("Errore nel ottenimento del numero del fascicolo: " + idFascicolo, ex);
    ...
    private String getIdFascicolo(String numerazioneGerarchica) throws SQLException {
        ...
        try (
            ...
            log.info("Query GetIdFascicolo: " + ps);
...
//            log.debug("Query setVicario: " + psSetVicario);
    ...
    public void execute(JobExecutionContext context) throws JobExecutionException {
        try {
            log.info("check Creazione Fascicoli Spaeciali");
            dbConnection = UtilityFunctions.getDBConnection();
            dbConnection.setAutoCommit(false);
            if (!alreadyExist()) {
                log.info("=========================== Avvio Creazione Fascicoli Spaeciali ===========================");
                create();
                dbConnection.commit();
                log.info("=========================== Fine Creazione Fascicoli Spaeciali ===========================");
            } else {
                log.info("ROLLBACK sulla transazione...");
                dbConnection.rollback();
                log.info("FATTO!");
                log.info("CLOSE della connessione...");
                dbConnection.close();
                log.info("FATTO!");
//                log.info("=========================== " + sequenceName + " ===========================");
                log.info("=========================== I fascicoli speciali sono già presenti per l'anno corrente ===========================");
            }
            ...
            try {
                ...
                log.info("Errore durante il RollBack: ", e);
            }
            log.info("Errore nella creazione dei fascicoli speciali: ", ex);
            log.info("=========================== Creazione dei Fascicoli speciali FALLITA ===========================");
        }
    }
