...
//            return;
                ...
                if (notDoneThisYear(dbConn)) {
                    ...
                    if (securityCheck(dbConn)) {
                        ...
                        if (existFascicoloSpeciale(dbConn)) {
                            ...
                            log.info("=========================== Avvio Duplicazione Fascicoli Attività ===========================");
                            ...
                            dbConn.commit();
                            log.info("=========================== Fine Duplicazione Fascicoli Attività ===========================");
                        } else {
    ...
    /**
     * Controllo di sicurezza: controlla che non esista nessun fascicolo creato
     * da questo servizio per quest anno. Torna "true" se i fascicoli
     * effettimavamente non sono stati ancora duplicati, "false" altrimenti
     *
     * @param dbConn
     * @return "true" se i fascicoli effettimavamente non sono stati ancora
    ...
    /**
     * Controlla che il servizio non sia stato già eseguito quest'anno. Per
     * farlo legge l'anno della data di fine dalla tabella bds_tools.servizi
     *
     * @param dbConn
     * @return "true" se il servizio non è stato eseguito quest'anno, "false"
     * altrimenti
     * @throws SQLException
     */
    private boolean notDoneThisYear(Connection dbConn) throws SQLException {
        ...
        try ( PreparedStatement ps = dbConn.prepareStatement(query)) {
            ...
            if (res.next()) {
                ...
                log.info(String.format("anno letto: %s", year));
    ...
    /**
     * aggiorna le date di avvio, termine del servizio, per aggiorarne solo una
     * passare null all'altra
     *
     * @param dataInizio
     * @param dataFine
     * @throws SQLException
    ...
    private void updateDataInizioDataFine(Connection dbConn, Timestamp dataInizio, Timestamp dataFine) throws SQLException {
        ...
        try ( PreparedStatement ps = dbConn.prepareStatement(query)) {
            ...
            ps.setString(3, getClass().getSimpleName());
            ps.setString(4, ID_APPLICAZIONE);
    ...
    /**
     * Estrare i fascicoli di tipo attività del livello passato, relativi
     * all'anno scorso e con stato "aperto" ("a") Nel caso di livello > 1 i
     * fascicoli saranno tutti i fascicoli di qualsiasi tipo (anche non
     * attività) e di qualsiasi anno discendenti di fascicoli radice di tipo
     * attività
     *
     * @param dbConn
     * @param ps passare un PreparedStatement, anche null; lo valorizza e lo usa
     * la funzione
     * @param livello
    ...
    /**
     * Esegue la duplicazione vera è propria. Passi della duplicazione: Cicla su
     * ogni livello, a partire da 1 fino a 3 (fascicoli, sotto fascicoli e
     * inserti) Per ogni livello estraggo i fascicoli del livello e per ognuno
     * inserisco un nuovo fascicolo in cui setto prima i campi che non posso
     * duplicare (es. id_fascicolo, guid_fascicolo, ecc.) e poi duplico tutti
     * gli altri campi (duplico tutti i campi ad eccezione di quelli indicati
     * nell'array COLUMN_TO_EXCLUDE) Durante la duplicazione salvo in una mappa
     * l'id del fascicolo che sto duplicando e l'id di quello duplicato, che
     * userò al giro successivo per settare l'id_fascicolo_padre sul fascicolo
     * figlio che duplicherò. A fine duplicazione, duplico anche i vicari.
     * Infine se il fascicolo è un fascicolo radice allora lo numero, usando il
     * servizio di numerazione.
     *
     * @param dbConn
     * @throws SQLException
     * @throws IOException
    ...
    private void duplicateFascicoli(Connection dbConn) throws SQLException, IOException, MalformedURLException, SendHttpMessageException, ServletException {
        ...
        Map<String,String> fascicoliVecchiNuoviLivelloUno = new HashMap();
...
//                    log.debug(String.format("writeMapIndex: %d", writeMapIndex));
                     ...
                     */
                    //mi preparo i la mappa di fascicoli di livello 1 sia vecchi che nuovi
...
//                            log.debug(String.format("id_fascicolo_padre duplicato = %s", idFascicoloPadre));
                            ...
                            fascicoliVecchiNuoviLivelloUno.put(idFascicoloSorgente, idFascicoloDuplicato);
                        }
        ...
        //ciclo sulla lista delle chiavi e per ogni chiave metto i vicari solo su quelli di livello uno perche poi ci penserà la spargivicari a metterli anche negli altri livelli
        for ( String idFascicoloSorgente: fascicoliVecchiNuoviLivelloUno.keySet()) {
            duplicateVicari(dbConn, idFascicoloSorgente, fascicoliVecchiNuoviLivelloUno.get(idFascicoloSorgente));
        }
        mettiPermessi(dbConn);
    }
    ...
    private void duplicateVicari(Connection dbConn, String idFascicoloSorgente, String idFascicoloDuplicato) throws SQLException {
        ...
        try ( PreparedStatement psVicari = dbConn.prepareStatement(queryVicari, ResultSet.TYPE_SCROLL_SENSITIVE, ResultSet.CONCUR_UPDATABLE, ResultSet.CLOSE_CURSORS_AT_COMMIT);) {
            ...
            log.info(String.format("eseguo la query: %s...", psVicari.toString()));
            ResultSet vicariRS = psVicari.executeQuery();
            if (vicariRS.last()) {
                int vicariNumber = vicariRS.getRow();
                log.info(String.format("vicari tovati: ", vicariNumber));
                vicariRS.beforeFirst();
                int vicariCont = 0;
                while (vicariRS.next() && vicariCont < vicariNumber) {
                    log.info(String.format("processing vicario row: %s --> %s", vicariRS.getString(1), vicariRS.getString(2)));
                ...
                log.info("non ci sono vicari");
            }
        }
    }

    private void mettiPermessi(Connection dbConn) throws SQLException {
        String queryOggetti =   "INSERT INTO procton_tools.oggetti (id_oggetto, tipo_oggetto) " +
                                "SELECT id_fascicolo, 4 " +
                                "FROM gd.fascicoligd f " +
                                "WHERE servizio_creazione = 'DuplicatoreFascicoliAttivita' AND anno_fascicolo = ? " +
                                "AND NOT EXISTS (SELECT 1 FROM procton_tools.oggetti oo WHERE id_oggetto = f.id_fascicolo)";
        
        String queryPermessi =  "INSERT INTO procton_tools.permessi " +
                                "(id_utente, id_oggetto, \"scope\", id_tipo_oggetto, attivo, subtree, " +
                                "provenienza, permesso, data_permesso, id_utente_permesso) " +
                                "SELECT p.id_utente, o.id, p.\"scope\", p.id_tipo_oggetto, p.attivo, p.subtree, 'GEDI', p.permesso, p.data_permesso, 'SANATORIA_FASCICOLI_ATTIVITA' " +
                                "FROM gd.fascicoligd f " +
                                "JOIN procton_tools.oggetti o ON o.id_oggetto = f.id_fascicolo " +
                                "JOIN procton_tools.oggetti o2 ON o2.id_oggetto = f.copiato_da " +
                                "JOIN procton_tools.permessi p ON p.id_oggetto = o2.id " +
                                "WHERE servizio_creazione = 'DuplicatoreFascicoliAttivita' AND anno_fascicolo = ?";
        
        try ( PreparedStatement psPermessi = dbConn.prepareStatement(queryOggetti, ResultSet.CLOSE_CURSORS_AT_COMMIT);) {
            psPermessi.setInt(1, currYear);
            log.info(String.format("eseguo la query %s", psPermessi.toString()));
            psPermessi.executeUpdate();
        
        }
        try ( PreparedStatement psPermessi = dbConn.prepareStatement(queryPermessi, ResultSet.CLOSE_CURSORS_AT_COMMIT);) {
            psPermessi.setInt(1, currYear);
            log.info(String.format("eseguo la query %s", psPermessi.toString()));
            psPermessi.executeUpdate();
