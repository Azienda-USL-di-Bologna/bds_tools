 ...
 */
public class UpdateFascicolo extends HttpServlet {

    private static final Logger log = LogManager.getLogger(UpdateFascicolo.class);
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        Boolean fascicoloAggiornato;  // da sistemare

        log.debug("REQUEST GR7 = " + request);
        ...
        try {
            ...
            try {
                ...
                fascicoloAggiornato = fascicoliUtilities.updateFascicolo(dbConn, ps, additionalData);

            } catch (Exception ex) {
                log.error("errore nell'update del fascicolo: ", ex);
                throw ex;
            } finally {
                if (ps != null) {
                    ps.close();
        ...
        response.addHeader("fascicoloAggiornato", fascicoloAggiornato ? "true" : "false");
        try (PrintWriter out = response.getWriter()) {
            out.print(fascicoloAggiornato);
        } catch (Exception ex) {
