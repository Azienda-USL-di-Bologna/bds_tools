...
public class LockUtilities {
    ...
    private String ambito = AMBITO_GD_DOC;
    ...
    public LockUtilities(String idOggetto, String tipoOggetto, String idSessione, String server, String ambito, String redisHost) {
        this.ambito = ambito;
        this.key = buildLockKey(idOggetto, tipoOggetto, server);
        this.value = buildDefaultLockValue(tipoOggetto, idSessione);
        this.timeoutSeconds = -1;
    ...
    /**
     * Rimuove il lock solo se il lock è dell'utente passato (l'utente è identificato dall'id_sessione)
     * @return 
     */
    public boolean removeUserLock() {
        String val = redisClient.getKey(key);
        log.info("value read: " + val);
        log.info("value excpected: + " + val);
